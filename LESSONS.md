...
4. **Domínio.** `up1.html` e `down1.html` roteiam via `https://DOMINIO-BR-PENDENTE/...` nos atributos `data-upsell-url`/`data-downsell-url`. O placeholder é deliberadamente inválido para falhar alto em vez de rotear silenciosamente para o domínio errado. `index.html` ainda não tem `og:url`/`og:image`.
